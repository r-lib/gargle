...
Versions of `secret_can_decrypt()` and `secret_read()` are defined here in gargle. `bq_auth()` is a function specific to bigrquery that loads a token for use downstream (in multiple tests, in this case).

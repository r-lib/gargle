...
If you are working with R in a web-based context, such as [RStudio Server](https://posit.co/download/rstudio-server/), [Posit Cloud](https://posit.cloud/), or [Posit Workbench](https://posit.co/products/enterprise/workbench/), your experience of browser-based auth flows will be different from those using R on their local machine.
...
Therefore, gargle has gained a new auth flow that we call "pseudo-OOB", which should allow casual users to continue to enjoy a low-friction auth experience even from RStudio Server, Posit Cloud, and Posit Workbench.
...
By default, OOB auth is used on RStudio Server, Posit Cloud, and Posit Workbench or if the option `"gargle_oob_default"` is set to `TRUE`.
...
Packages that use a built-in tidyverse OAuth client (googledrive, googlesheets4, and bigrquery) should automatically select a "web" client on RStudio Server, Posit Cloud, and Posit Workbench and an "installed" client otherwise.
...
* 2023 February 1: Conventional OOB will no longer work for the tidyverse OAuth
  client. Users on RStudio Server, Posit Workbench, and Posit Cloud will need to
  use the pseudo-OOB flow or, as has always been true, non-interactive methods
  of auth, like using a service account token or uploading a user token obtained
  elsewhere.

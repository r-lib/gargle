...
The help for `cred_funs_list()` is a good place to learn more.
...
To see more information about what gargle is up to, set the option named "gargle_verbosity" to "debug".
...
The `scopes`, `app`, and `package` are generally provided by the API wrapper function that is mediating the calls to `token_fetch()`.
...
gargle looks in the cache specified by `gargle_oauth_cache()` for a token that has these scopes, this app, and the Google identity specified by `email`.
By default `email` is `NA`, so we might find one or more tokens that have the necessary scopes and app.

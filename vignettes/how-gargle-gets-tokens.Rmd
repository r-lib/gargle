...
The fourth function tried is `credentials_user_oauth2()`. Here's how a call to `token_fetch()` might work:
...
`credentials_byo_oauth2()` checks that `token` is of class `httr::Token2.0` and that it appears to be associated with Google. If `token` is not provided or if it doesn't satisfy these requirements, we fail and `token_fetch()`'s execution moves on to the next function in the registry.

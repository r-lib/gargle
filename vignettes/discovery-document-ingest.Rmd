...
You can write less code and safer code, in return for a modest investment in studying your target API. That is the "Discovery Document Ingest" from the title of this vignette.
...
`request_develop(endpoint, params, base_url)`: a.k.a. The Smart One.
  ...
  * Returns request data in a form that anticipates the `httr::VERB()` call that
    is on the horizon.
...
`request_make(x, ..., user_agent)`: actually makes the HTTP request.
  ...
  * Consults `x$method` to determine which `httr::VERB()` to call, then calls it with the rest of `x` and `...` passed as arguments.
...
The gargle package ships with some functions and scripts to facilitate the ingest of a Discovery Document. You can file these files in the gargle installation like so:

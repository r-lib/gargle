...
Therefore, the introduction of pseudo-oob meant that gargle had to learn about different OAuth client types.
...
Note the class of `client`, the new `type` field, and the `redirect_uris`.
...
Here are the changes in `AuthState`:
...
The wrapper package might export a user-facing function, `PKG_auth_configure()` that allows the user to configure their own OAuth client or API key.
The `PKG_auth_configure()` function typically exposes an argument for the OAuth client and it makes sense to rename this from `app` to `client`.
It is also typical to export a function that reveals the user's configured OAuth client.
It makes sense to rename this from `PKG_oauth_app()` to `PKG_oauth_client()`.
...
The approach above follows various conventions explained in the "Arguments" section of `vignette("communicate", package = "lifecycle")`.
(The `usethis::use_lifecycle()` function does the recommended setup, to start using the lifecycle package.)
The first argument of `PKG_auth_configure()` is now `client` and `app` is moved to the end, with default value `deprecated()`.
In the body of `drive_auth_configure()`, we'll issue a deprecation warning for explicit usage of the `app` argument and then use that input as the `client` instead.
Instead of calling `.auth$set_app()`, we use `.auth$set_client()` instead.
A call to `drive_oauth_app()` results in a deprecation warning, then a call to `drive_oauth_client()` instead.

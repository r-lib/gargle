...
PREFIX_auth_details <- function(.data = list(
  ...
  glue_data_lines(c(
    ...
    "    browser to choose the identity and give consent. If the email does",
    "    not contain the `@` symbol, it is interpreted as specifying the",
    "    domain. This means that a script containing `email = \"example.com\"`",
    "    can be run without further tweaks by both `alice@example.com` and",
    "    `bob@example.com`.",
